...
def perception(r_px, x_px, y_px):
    ...
    yellow_low = (34, 132, 177)
    yellow_high = (123, 255, 255)
    ...
    green_low_hsv = (40, 100, 0)
    green_high_hsv = (95, 255, 255)
    ...
    while True:
        ...
        if time_passed >= dt:
            ...
            #otherwise, convert pixel location to angle and distance
                ...
                theta = -np.arctan((2*(P_xc/2 - x_px.value))/(P_xc*np.tan((pi/2) - alpha)))
                #phi is the vertical angle of the object from the horizontal plane coming straight out of the car's nose
                phi = -np.arctan((2*abs(P_yc/2 - y_px.value))/(P_yc*np.tan((pi/2) - beta)))
            ...
            print("Distance: ", object_distance, " Angle: ", theta*180/pi)
